...
base_prob_train = lr.predict_proba(X_base_train_scaled)[:,1]
base_prob_test = lr.predict_proba(X_base_test_scaled)[:,1]

# 3. 随机森林（动态特征）
rf = RandomForestClassifier(n_estimators=100, random_state=42)
rf.fit(X_dyn_train_scaled, y_train)
dyn_prob_train = rf.predict_proba(X_dyn_train_scaled)[:,1]
dyn_prob_test = rf.predict_proba(X_dyn_test_scaled)[:,1]

# 4. stacking融合
stack_train = np.vstack([base_prob_train, dyn_prob_train]).T
stack_test = np.vstack([base_prob_test, dyn_prob_test]).T
meta_lr = LogisticRegression(max_iter=1000)
meta_lr.fit(stack_train, y_train)
stack_prob = meta_lr.predict_proba(stack_test)[:,1]
...
def ks_score(y_true, y_prob):
    ...
    return max(tpr - fpr)
print(f'Stacking融合模型KS值: {ks_score(y_test, stack_prob):.4f}')
...
scores = prob_to_score(stack_prob)
